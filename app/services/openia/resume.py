...
async def generate_general_resume(messages, users, chatresume_id) -> dict:
     # Combina todos los mensajes en un solo texto
    chat = "\n".join(f"{users[message['user']]['name']}: {message['text']}" for message in messages if message['user'] != chatresume_id)
    ...
    print(user_info)
    prompt = f"The following is a conversation from a Slack channel.\nThe participants are:\n\n{user_info}\nthe chat is:\n{chat}\n\nAs an AI language model, your task is to provide a detailed summary that encapsulates the key points, decisions, and action items from this conversation."
    print(prompt)
